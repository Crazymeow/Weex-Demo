...
<template>
    ...
    <div>
		...
		<div class="tab scroller-box sticky" style="position: static;">
	
		    <div class="app-scroller" v-iscroll="opts" ref="tab">
		        <!-- content，需要在列表外部包围一个div -->
			    	<div class="scroller-items hor-view">
			    		
		            <a v-for="(tab,idx) in list" :ref="'tab'+idx" :class="['item',tabIndex==idx?'cur':'']"@click="scrollToTab(idx,tab.id)" name="tab.trickPoint">{{tab.name}}</a>
