...
function protocol(url, http) {
    ...
    var ptcol = weex.config.bundleUrl
    var _url = ''
 ...
 */
function getEnvName() {
    var _hostName = weex.config.bundleUrl,
        envName;
    if (dectect().Web) {
...
function getQueryString(name, url) {
    var key = {}
    var location = url ? decodeURIComponent(url) : weex.config.bundleUrl
    if (location.indexOf('?') <= -1) {
        return ''
    }
    var qArr = location.split("?")[1].split("&")
...
function isApp() {
    if (typeof(weex.config.env.userAgent) == 'string') {
